...
PlayerWindow::PlayerWindow(QWidget *parent) : QWidget(parent)
{
    ...
    connect(this, SIGNAL(stopMedia()), m_player, SLOT(stop()));
...
void PlayerWindow::playDefault(QString filename)
{
    if (filename.isEmpty())
        return;
...
void PlayerWindow::playList(Playlist *playlist)
{
    qDebug()<<playlist->getFilePath() + ((playlist->getPlayList())).at(video_index);
    playDefault(playlist->getFilePath() + ((playlist->getPlayList())).at(video_index));
    m_playlist = playlist;
}
...
void PlayerWindow::changeMedia()
{
    video_index ++;
    m_playlist->updatePlayList();
    if(video_index >= ((m_playlist->getPlayList())).count()){
       video_index = 0;
    }
    qDebug() <<"video_index"<<video_index;
    playDefault(m_playlist->getFilePath() + ((m_playlist->getPlayList())).at(video_index));
}
...
void PlayerWindow::updateSlider(qint64 value)
{
    ...
    //qDebug() << "updateslider value:" << value <<"/"<<m_player->duration();
    if(m_player->duration() - value < 1000 ){
        qDebug() <<"changeMedia";
       emit stopMedia();
       changeMedia();
    }
